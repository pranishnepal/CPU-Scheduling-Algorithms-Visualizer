...
/**
 * @return the shortest task in the LinkedList using bubble sort: scans from the head each time to find the shortest.
 */
Task *getShortestTask() {
    struct node *currNode = listHead;
    Task *currShortestTask = currNode->task;
    int currShortestBurst = currShortestTask->burst;

    for (; currNode != NULL; currNode = currNode->next) {
        Task *currNodeTask = currNode->task;
        int currNodeTaskBurst = currNodeTask->burst;

        if (currNodeTaskBurst <= currShortestBurst) {
            currShortestTask = currNodeTask;
            currShortestBurst = currNodeTaskBurst;
        }
    }

    return currShortestTask;
}
