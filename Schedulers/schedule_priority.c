...
void schedule() {
    ...
    while (listHead != NULL) {
        Task *currTask = getHighestPriorityTask();
...
/**
 * Assumption: higher priority value -> higher priority
 * @return the highest priority task in the LinkedList using bubble sort: scans from the head each time to find the shortest.
 * In case of tie in priority values, returns the earlier Task (FIFO Order).
 */
Task *getHighestPriorityTask() {
    struct node *currNode = listHead;
    Task *currHighestPriorityTask = currNode->task;
    int currHighestPriorityValue = currHighestPriorityTask->priority;

    for (; currNode != NULL; currNode = currNode->next) {
        Task *currNodeTask = currNode->task;
        int currNodeTaskPriorityValue = currNodeTask->priority;

        if (currNodeTaskPriorityValue >= currHighestPriorityValue) {
            currHighestPriorityTask = currNodeTask;
            currHighestPriorityValue = currNodeTaskPriorityValue;
        }
    }

    return currHighestPriorityTask;
}
